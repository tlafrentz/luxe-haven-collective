...
function ShellNavigation({ experience, navigation, pathname, collapsed, onNavigate, onToggle }: { experience: PlatformExperience; navigation: readonly NavigationItem[]; pathname: string; collapsed: boolean; onNavigate: () => void; onToggle: () => void }) {
  ...
  return <div className="flex h-full flex-col bg-[#0b0d0e]">
    ...
    <nav aria-label={experience === "client-workspace" ? "HPM workspace navigation" : "Operations Console navigation"} className="flex-1 overflow-x-hidden overflow-y-auto px-3 py-5">
      {groups.map(group => <section key={group} aria-labelledby={`nav-${group}`} className="mb-5"><h2 id={`nav-${group}`} className={collapsed || group === "home" ? "sr-only" : "mb-2 px-3 text-[11px] font-semibold uppercase tracking-[0.2em] text-amber-200"}>{groupLabels[group]}</h2><div className="space-y-1">{navigation.filter(item => item.group === group).map(item => <NavigationEntry key={item.id} item={item} pathname={pathname} collapsed={collapsed} experience={experience} parentActive={activeHierarchy.has(item.id)} onNavigate={onNavigate} />)}</div></section>)}
    </nav>
...
function NavigationEntry({ item, pathname, collapsed, experience, parentActive, onNavigate }: { item: NavigationItem; pathname: string; collapsed: boolean; experience: PlatformExperience; parentActive: boolean; onNavigate: () => void }) {
  ...
  const classes = ["group/nav relative flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-left outline-none transition-colors focus-visible:ring-2 focus-visible:ring-teal-400 focus-visible:ring-offset-2 focus-visible:ring-offset-stone-950", active ? experience === "operations-console" ? "border-l-2 border-amber-400 bg-white/[0.08] text-stone-200" : "bg-white/[0.10] text-stone-200 ring-1 ring-white/10" : parentActive ? "text-stone-300" : disabled ? "cursor-default text-stone-500" : "text-stone-400 hover:bg-white/[0.06] hover:text-stone-200"].join(" ");
