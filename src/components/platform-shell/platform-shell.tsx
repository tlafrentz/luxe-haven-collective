...
function NavigationEntry({ item, pathname, collapsed, experience, parentActive, onNavigate }: { item: NavigationItem; pathname: string; collapsed: boolean; experience: PlatformExperience; parentActive: boolean; onNavigate: () => void }) {
  ...
  const classes = ["group/nav relative flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-left outline-none transition-colors focus-visible:ring-2 focus-visible:ring-teal-400 focus-visible:ring-offset-2 focus-visible:ring-offset-stone-950", active ? experience === "operations-console" ? "border-l-2 border-amber-400 bg-white/[0.08] text-white" : "bg-white/[0.10] text-white ring-1 ring-white/10" : parentActive ? "text-stone-200" : disabled ? "cursor-default text-stone-500" : "text-stone-300 hover:bg-white/[0.06] hover:text-white"].join(" ");
