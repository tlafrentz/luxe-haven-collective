...
function ShellNavigation({ experience, navigation, pathname, collapsed, onNavigate, onToggle }: { experience: PlatformExperience; navigation: readonly NavigationItem[]; pathname: string; collapsed: boolean; onNavigate: () => void; onToggle: () => void }) {
  ...
  return <div className="flex h-full flex-col bg-[#0b0d0e]">
    <div className="border-b border-white/10 px-5 py-5"><div className="flex items-center justify-between"><Link href={experience === "client-workspace" ? "/dashboard" : "/admin"} onClick={onNavigate} className="min-w-0 rounded outline-none focus-visible:ring-2 focus-visible:ring-teal-400"><span className="block truncate text-xs font-semibold uppercase tracking-[0.22em] text-amber-200">Luxe Haven</span>{!collapsed ? <span className="mt-1 block truncate text-xl font-semibold">{experience === "client-workspace" ? "HPM Workspace" : "Operations Console"}</span> : null}</Link><button type="button" onClick={onToggle} aria-label={collapsed ? "Expand navigation" : "Collapse navigation"} className="hidden h-10 w-10 items-center justify-center rounded-xl border border-white/10 text-stone-300 outline-none hover:bg-white/10 focus-visible:ring-2 focus-visible:ring-teal-400 lg:flex">{collapsed ? "→" : "←"}</button></div>{!collapsed && experience === "client-workspace" ? <div className="mt-4 rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2"><span className="block text-[10px] font-semibold uppercase tracking-[0.14em] text-stone-500">Current workspace</span><span className="mt-1 block truncate text-xs font-semibold text-stone-200">Luxe Haven Collective</span></div> : null}</div>
    <nav aria-label={experience === "client-workspace" ? "HPM workspace navigation" : "Operations Console navigation"} className="flex-1 overflow-x-hidden overflow-y-auto px-3 py-5">
      {groups.map(group => <section key={group} aria-labelledby={`nav-${group}`} className="mb-5"><h2 id={`nav-${group}`} className={collapsed || group === "home" ? "sr-only" : "mb-2 px-3 text-[11px] font-semibold uppercase tracking-[0.2em] text-stone-500"}>{groupLabels[group]}</h2><div className="space-y-1">{navigation.filter(item => item.group === group).map(item => <NavigationEntry key={item.id} item={item} pathname={pathname} collapsed={collapsed} experience={experience} parentActive={activeHierarchy.has(item.id)} onNavigate={onNavigate} />)}</div></section>)}
    </nav>
...
function NavigationEntry({ item, pathname, collapsed, experience, parentActive, onNavigate }: { item: NavigationItem; pathname: string; collapsed: boolean; experience: PlatformExperience; parentActive: boolean; onNavigate: () => void }) {
  ...
  if (item.kind === "group") return <div className={["mt-3 flex min-h-8 items-center px-3 text-[11px] font-semibold uppercase tracking-[0.16em]", parentActive ? "text-stone-200" : "text-stone-500"].join(" ")} aria-expanded="true"><span>{item.label}</span><span className="sr-only">, {item.description}</span></div>;
  ...
  const content = <><span className={["relative z-10 flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border text-[10px] font-semibold", item.level === 3 ? "scale-90" : "", active ? experience === "operations-console" ? "border-amber-400/40 bg-amber-400/10 text-amber-200" : "border-white bg-white text-stone-950" : "border-white/10 bg-[#151719] text-stone-300"].join(" ")}>{item.icon ? iconLabels[item.icon] : "•"}</span>{collapsed ? <><span className="sr-only">{item.label}{status ? `, ${status}` : ""}</span><span role="tooltip" className="pointer-events-none absolute left-full z-50 ml-3 hidden min-w-max rounded-lg bg-stone-800 px-3 py-2 text-xs font-semibold text-white shadow-xl group-hover/nav:block group-focus-visible/nav:block">{item.label}</span></> : <span className="min-w-0 flex-1"><span className="flex items-center justify-between gap-2"><span className="block text-sm font-semibold">{item.label}</span>{status ? <span aria-hidden="true" className="rounded-full border border-white/15 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-stone-400">{status}</span> : null}</span>{item.description && experience === "operations-console" ? <span className="mt-0.5 block text-xs leading-4 text-stone-500">{item.description}</span> : null}{status ? <span className="sr-only">Availability: {status}</span> : null}</span>}</>;
  const inset = !collapsed ? item.level === 3 ? "ml-8 w-[calc(100%-2rem)] border-l border-white/10" : item.level === 2 ? "ml-4 w-[calc(100%-1rem)]" : "" : "";
  const classes = ["group/nav relative flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-left outline-none transition-colors focus-visible:ring-2 focus-visible:ring-teal-400 focus-visible:ring-offset-2 focus-visible:ring-offset-stone-950", inset, active ? experience === "operations-console" ? "border-l-2 border-amber-400 bg-white/[0.08] text-white" : "bg-white/[0.10] text-white ring-1 ring-white/10" : parentActive ? "text-stone-200" : disabled ? "cursor-default text-stone-500" : "text-stone-300 hover:bg-white/[0.06] hover:text-white"].join(" ");
...
export function pageDetails(pathname: string, experience: PlatformExperience): { eyebrow: string; title: string; breadcrumbs: readonly Crumb[] } {
  ...
  if (pathname.startsWith("/guidebooks")) return { eyebrow: "Services · Guest experience", title: "Guidebook Studio", breadcrumbs: [{ id: "home", label: "Home", href: "/dashboard" }, { id: "current", label: "Guidebook Studio", current: true }] };
