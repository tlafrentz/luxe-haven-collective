...
export function LearningHeader({ title, description }: { title: string; description: string }) {
  return <header><p className="text-xs font-semibold uppercase tracking-[.2em] text-teal-700">Learn · Remember · Improve</p><h1 className="mt-2 text-4xl font-semibold tracking-tight text-stone-950">{title}</h1><p className="mt-3 max-w-3xl text-stone-600">{description}</p><nav aria-label="Learning workspace" className="mt-6 flex flex-wrap gap-2 text-sm"><Nav href="/dashboard/learning">Dashboard</Nav><Nav href="/dashboard/learning/lessons">Knowledge base</Nav><Nav href="/dashboard/learning/reviews">Outcome reviews</Nav><Nav href="/dashboard/learning/gaps">Knowledge gaps</Nav><Nav href="/dashboard/learning/health">Knowledge health</Nav></nav></header>;
}
...
export function LessonCard({ lesson }: { lesson: LearningWorkspaceLesson }) {
  return <article className="rounded-2xl border border-stone-200 bg-white p-5"><div className="flex flex-wrap items-center gap-2"><Tag>{lesson.category}</Tag><Tag>{lesson.maturity}</Tag><Tag>{lesson.confidence} confidence</Tag>{lesson.contradictionState!=="none"?<Tag>Contradiction {lesson.contradictionState}</Tag>:null}</div><h3 className="mt-4 text-lg font-semibold"><Link className="hover:text-teal-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-600" href={`/dashboard/learning/lessons/${lesson.id}`}>{lesson.title}</Link></h3><p className="mt-2 text-sm leading-6 text-stone-600">{lesson.statement}</p><p className="mt-4 text-xs text-stone-500">{lesson.sourceReviewIds.length} supporting reviews · {lesson.evidenceCount} evidence references · revision {lesson.revision}</p><div className="mt-3 flex flex-wrap gap-2">{lesson.applicability.map((item,index)=><span key={`${item.dimension}:${item.referenceId??item.value}:${index}`} className="rounded-md bg-teal-50 px-2 py-1 text-xs text-teal-900">{item.dimension}: {item.value??item.referenceId}</span>)}</div></article>;
}
export function ReviewRow({ review }: { review: LearningWorkspaceReview }) {
  return <tr className="border-t"><td className="p-4"><Link className="font-semibold hover:text-teal-800" href={`/dashboard/learning/reviews/${review.id}`}>Review {review.seriesId}</Link><span className="block text-xs text-stone-500">Plan revision {review.planRevision} · Review revision {review.revision}</span></td><td className="p-4 capitalize">{review.status.replaceAll("-"," ")}</td><td className="p-4 capitalize">{review.summaryStatus?.replaceAll("-"," ")??"Pending"}</td><td className="p-4 capitalize">{review.confidence.replaceAll("-"," ")}</td><td className="p-4">{review.completedAt?new Date(review.completedAt).toLocaleDateString():"Not completed"}</td></tr>;
}
export function GapCard({ gap }: { gap: KnowledgeGap }) {
  return <article className="rounded-2xl border border-amber-200 bg-amber-50 p-5"><Tag>{gap.severity} priority</Tag><h3 className="mt-3 font-semibold">{gap.title}</h3><p className="mt-2 text-sm text-stone-700">{gap.detail}</p><Link className="mt-4 inline-flex text-sm font-semibold text-teal-800" href={gap.href}>Investigate gap →</Link></article>;
}
