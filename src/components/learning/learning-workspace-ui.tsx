...
export function ContradictionCard({ contradiction }: { contradiction: LearningContradictionSummary }) {
  return <article className="rounded-2xl border border-rose-200 bg-rose-50 p-5"><Tag>{contradiction.state}</Tag><h3 className="mt-3 font-semibold">Conflicting knowledge retained</h3><p className="mt-2 text-sm text-stone-700">{contradiction.rationale}</p><div className="mt-4 grid gap-2 text-sm sm:grid-cols-2"><Link className="rounded-lg bg-white p-3 font-mono text-xs" href={`/dashboard/learn/lessons/${contradiction.firstLessonId}`}>Lesson A: {contradiction.firstLessonId}</Link><Link className="rounded-lg bg-white p-3 font-mono text-xs" href={`/dashboard/learn/lessons/${contradiction.secondLessonId}`}>Lesson B: {contradiction.secondLessonId}</Link></div></article>;
}
