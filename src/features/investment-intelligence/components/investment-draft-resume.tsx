...
export function InvestmentDraftResume({ ownerScope, emptyState }: { ownerScope?: string; emptyState: ReactNode }) {
  const [draft, setDraft] = useState<DraftSummary | null>(null);
  const [resolved, setResolved] = useState(!ownerScope);
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      try {
        ...
        if (raw) {
          ...
          if (stored.schemaVersion === "investment-workspace-draft.v1" && stored.ownerScope === ownerScope && (strategy === "purchase" || strategy === "rental-arbitrage")) {
