...
// The original local version collided with a remote migration and was replaced
// by this uniquely versioned repair. The repair is now the canonical source.
const migration = repair;
...
describe("GB-PLAT-001 persistence", () => {
  ...
  it("enables RLS with workspace and property authorization", () => {
    ...
    expect(migration).not.toMatch(
      /grant (insert|update|delete) on public\.property_capability_enrollments to authenticated/i,
    );
  });

  it("does not infer HPM from a generic property record", () => {
    expect(migration).toContain(
      "exists(select 1 from public.external_properties",
    );
    expect(migration).toContain("exists(select 1 from public.bookings");
    expect(migration).not.toMatch(
      /select distinct p\.owner_id,p\.id,'hpm'[\s\S]*from public\.properties p\s*on conflict/,
    );
  });
