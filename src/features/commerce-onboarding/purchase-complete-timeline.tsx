...
function TimelineStep({ label, state }: { label: string; state: TimelineState }) {
  return (
    <li className="flex items-center gap-3">
      ...
      <span className={state === "pending" ? "text-stone-400" : "font-semibold text-[#171c19]"}>
        {label}
      </span>
    </li>
  );
  ...
  return (
    <div className="mx-auto max-w-xl rounded-2xl border border-[#dce2dd] bg-white p-8">
      ...
      <p className="mt-3 text-center text-sm text-stone-600">
        Welcome to the Hospitality Performance Platform. Your subscription is active and your
        workspace is being prepared.
      </p>
      <ol className="mt-8 space-y-4">
        <TimelineStep label="Payment received" state={paymentState} />
        <TimelineStep label="Workspace created" state={workspaceState} />
        <TimelineStep label="Activation ready" state={activationState} />
      </ol>
      {result ? (
        <dl className="mt-8 rounded-xl bg-stone-50 p-5 text-left text-sm">
          <div>
            <dt className="text-xs uppercase tracking-wider text-stone-500">Order</dt>
            <dd className="mt-1 font-semibold">{result.orderNumber}</dd>
          </div>
          <div className="mt-4">
            <dt className="text-xs uppercase tracking-wider text-stone-500">Product</dt>
