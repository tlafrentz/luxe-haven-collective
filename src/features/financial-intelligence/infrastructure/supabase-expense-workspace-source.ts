...
export class SupabaseExpenseWorkspaceSource{
  async read(input:{workspaceId:string;propertyIds:readonly string[];from:string;to:string;basis:FinancialExpenseBasis}):Promise<ExpenseWorkspace>{
    ...
    const [transactions,properties]=await Promise.all([
      input.propertyIds.length?client.from("financial_transactions").select("id,account_id,property_id,amount_minor,currency,measurement,effective_date,effective_to,frequency,source_provider,source_external_id,status,financial_accounts(name,subcategory),properties(name)").eq("workspace_id",input.workspaceId).in("property_id",[...input.propertyIds]).eq("measurement",input.basis).gte("effective_date",input.from).lte("effective_date",input.to).order("effective_date",{ascending:false}):Promise.resolve({data:[],error:null}),
