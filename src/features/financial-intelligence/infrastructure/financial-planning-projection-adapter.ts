...
export class FinancialPlanningProjectionAdapter implements FinancialPlanningReader{
  constructor(private readonly access:WorkspaceAccessContext,private readonly source:FinancialSource,private readonly catalog:FinancialPropertyCatalog,private readonly repository:FinancialPlanningRepository=emptyFinancialPlanningRepository){}
  async read(query:GetFinancialPlanningQuery){
    ...
    const [budget,forecast,scenarios,propertyPlanRecords,propertyActuals]=await Promise.all([
