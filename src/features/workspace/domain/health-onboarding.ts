...
export function buildWorkspaceHealth(input: WorkspaceReadinessInput, now = new Date()): CanonicalWorkspaceHealth {
  ...
  if (["degraded","unusable"].includes(input.operationalQuality)) add(issue("operational_quality_degraded","operational-data","action-required","Operational records require attention","One or more included properties have incomplete or conflicting operational records.","Affected metrics remain limited until the listed data-quality issues are resolved.",["Bookings","Home","Executive Intelligence"],{ type:"review",label:"Review data quality",href:"/dashboard/workspace/health" },false));
