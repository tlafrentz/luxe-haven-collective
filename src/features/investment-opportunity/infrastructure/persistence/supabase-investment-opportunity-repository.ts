...
import { InvestmentOpportunityError, type InvestmentOpportunityPage, type InvestmentOpportunityRepository, type InvestmentOpportunityRepositoryQuery, type InvestmentOpportunitySaveOptions, type InvestmentOpportunitySaveResult } from "../../application";
...
export interface SupabaseInvestmentOpportunityGateway {
  saveAtomic(payload: ReturnType<typeof toInvestmentOpportunityPersistence>, expectedVersion?: number, commandId?: string, options?: InvestmentOpportunitySaveOptions): Promise<InvestmentOpportunitySaveResult>;
  ...
  findSaveReceipt?(commandId: string, payloadHash: string): Promise<InvestmentOpportunitySaveResult | null>;
}
export class SupabaseInvestmentOpportunityRepository implements InvestmentOpportunityRepository {
  constructor(private readonly gateway: SupabaseInvestmentOpportunityGateway, private readonly workspaceId?: string, private readonly actorProfileId?: string) {}
  ...
  async save(opportunity: InvestmentOpportunity, expectedVersion?: number, idempotencyKey?: string, options?: InvestmentOpportunitySaveOptions) { try {
    const mapped = toInvestmentOpportunityPersistence(opportunity);
    const payload = this.workspaceId && this.actorProfileId ? { ...mapped, opportunity: { ...mapped.opportunity, owner_id: this.actorProfileId, workspace_id: this.workspaceId } } : mapped;
    return await this.gateway.saveAtomic(payload, expectedVersion, idempotencyKey, options);
  } catch (cause) { if (isSerializationFailure(cause)) throw new InvestmentOpportunityError("CONCURRENT_OPPORTUNITY_MODIFICATION", "The opportunity was modified concurrently.", cause); throw new InvestmentOpportunityError("OPPORTUNITY_PERSISTENCE_FAILED", "Investment opportunity persistence failed.", cause); } }
  ...
  async findSaveReceipt(commandId: string, payloadHash: string) { return this.gateway.findSaveReceipt?.(commandId, payloadHash) ?? null; }
