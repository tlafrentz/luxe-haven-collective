...
export function projectActionCenterQueue(actions: readonly PlatformAction[], viewer: ActionCenterViewer, now = new Date()): ActionCenterQueue {
  ...
  const allActions = projected.filter((action) => action.status !== "archived").sort(compareActions);
  const activeActions = projected.filter((action) => !["cancelled", "archived", "completed"].includes(action.status)).sort(compareActions);
  const completedActions = projected.filter((action) => ["completed", "cancelled"].includes(action.status)).sort((a, b) => b.updatedAt.getTime() - a.updatedAt.getTime());
  return Object.freeze({ summary: Object.freeze({ total: allActions.length, ready: projected.filter((a) => a.status === "ready").length, inProgress: projected.filter((a) => a.status === "in-progress").length, blocked: projected.filter((a) => a.status === "blocked").length, awaitingReview: projected.filter((a) => a.status === "awaiting-review").length, failed: projected.filter((a) => a.status === "failed").length, overdue: projected.filter((a) => a.isOverdue).length, unassigned: projected.filter((a) => !a.assignee && !["draft", "completed", "cancelled", "archived"].includes(a.status)).length, completed: projected.filter((a) => a.status === "completed").length }), allActions: Object.freeze(allActions), activeActions: Object.freeze(activeActions), completedActions: Object.freeze(completedActions), isEmpty: projected.length === 0 });
}
