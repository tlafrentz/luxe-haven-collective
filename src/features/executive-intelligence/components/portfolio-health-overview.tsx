    ...
    <section>
      ...
      <div className="mt-5 rounded-3xl border border-stone-200 bg-stone-100/60 p-5">
        <div className="grid gap-4 sm:grid-cols-3 xl:grid-cols-1 2xl:grid-cols-3">
          <div className="min-w-0"><p className="text-xs text-stone-500">Health score</p><p className="mt-1 text-xl font-semibold text-stone-950">{health.score ?? "Unavailable"}</p></div>
          <div className="min-w-0"><p className="text-xs text-stone-500">Confidence</p><p className="mt-1 text-xl font-semibold text-stone-950">{health.confidence === null ? "Unavailable" : `${health.confidence}%`}</p></div>
          <div className="min-w-0"><p className="text-xs text-stone-500">Status</p><p className="mt-1 text-lg font-semibold text-stone-950">{statusLabel(health.status)}</p></div>
        </div>
