...
import { HelpTooltip } from "@/components/application-layout";
...
export function ExecutiveDataQuality({ dataQuality }: Readonly<{ dataQuality: ExecutiveDataQualitySummary }>) {
  return (
    <section className="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm">
      ...
      <p className="mt-4 inline-flex items-center text-xs text-stone-500">Overall confidence: {dataQuality.confidence === null ? "Unavailable" : `${dataQuality.confidence}%`}<HelpTooltip label="confidence">Confidence reflects how complete, current, and well-supported the available evidence is. It is not a performance score.</HelpTooltip></p>
