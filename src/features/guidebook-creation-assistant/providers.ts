...
export class DirectOpenAiCreationProvider implements ExtractionProvider,GenerationProvider{
  ...
  async generate(input:Parameters<GenerationProvider["generate"]>[0]){
    ...
    let output;try{output=await this.request("generation",model,[{type:"input_text",text}],input.correlationId,input.signal,12000)}catch(error){await retainFailureEvidence(input.evidence,error);throw error}const evidenceId=await retainEvidence(input.evidence,evidenceFromOutput(output));
    try{return{proposal:parseResponse(output.text,guidebookProposalSchema),requestId:output.requestId,usage:output.usage,evidenceId}}catch(error){await input.evidence?.recordOutcome?.(evidenceId,{classification:"application_schema_failure",schemaValidationPaths:schemaPaths(error)});throw new CreationProviderError("invalid_output","The creation provider returned an invalid result.",false,null,output.usage,output.requestId)}
  }
...
function parseResponse<T>(text:string,schema:{parse(value:unknown):T}){return schema.parse(JSON.parse(text))}
