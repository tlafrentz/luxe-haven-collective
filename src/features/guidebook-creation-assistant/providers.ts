...
export class DirectOpenAiCreationProvider implements ExtractionProvider,GenerationProvider{
  ...
  private async request(stage:DirectStage,model:string,content:Record<string,unknown>[],correlationId:string,outerSignal:AbortSignal,maxOutputTokens:number){
    ...
    try{const response=await this.fetcher(OPENAI_ENDPOINT,{method:"POST",headers:{authorization:`Bearer ${this.config.apiKey}`,"content-type":"application/json","x-client-request-id":correlationId,...(this.config.projectId?{"OpenAI-Project":this.config.projectId}:{})},body:JSON.stringify({model,store:false,input:[{role:"user",content}],text:{format:{type:"json_object"}},reasoning:{effort:stage==="extraction"?"minimal":"low"},max_output_tokens:maxOutputTokens}),signal:controller.signal});if(!response.ok)throw classifyStatus(response.status);const body=await response.json()as Record<string,unknown>,usage=pricedUsage(body.usage,model,stage,performance.now()-started);if(Number(usage.calculated_cost_usd??0)>GUIDEBOOK_COST_CEILING_USD)throw new CreationProviderError("terminal","The guidebook cost ceiling was exceeded.",false);if(body.status==="incomplete"){const details=body.incomplete_details as Record<string,unknown>|undefined,reason=details?.reason;throw new CreationProviderError("invalid_output","The creation provider returned an incomplete result.",reason==="max_output_tokens")}const text=responseText(body);if(!text)throw new CreationProviderError("invalid_output","The creation provider returned an invalid result.",false);return{text,requestId:String(body.id??correlationId),httpStatus:response.status,openaiRequestId:response.headers.get("x-request-id"),model:typeof body.model==="string"?body.model:model,usage}}catch(error){if(error instanceof CreationProviderError)throw error;if(controller.signal.aborted)throw new CreationProviderError("timeout","The creation provider timed out.",true);throw new CreationProviderError("unavailable","The creation provider is unavailable.",true)}finally{clearTimeout(timeout);outerSignal.removeEventListener("abort",abort)}
  }
}
