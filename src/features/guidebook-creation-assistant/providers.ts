import "server-only";
import { z } from "zod";
import { extractionResultSchema,guidebookProposalSchema,type CreationSource,type ExtractedFact,type ExtractedNarrativeSection,type ExtractionResult,type GuidebookProposal } from "./domain";
...
export interface GenerationProvider{readonly key:string;generate(input:Readonly<{facts:readonly ExtractedFact[];narrativeSections:readonly ExtractedNarrativeSection[];templateVersionId:string;allowedComponents:ReadonlyMap<string,string>;instructions:Readonly<Record<string,string|boolean>>;correlationId:string;signal:AbortSignal;evidence?:ProviderEvidenceContext}>):Promise<Readonly<{proposal:GuidebookProposal;requestId:string;usage?:ProviderUsage;evidenceId?:string}>>}
...
const guidebookProviderContentSchema=guidebookProposalSchema.omit({factIds:true,narrativeSectionIds:true,componentVersions:true});
...
export class DirectOpenAiCreationProvider implements ExtractionProvider,GenerationProvider{
  ...
  async generate(input:Parameters<GenerationProvider["generate"]>[0]){
    ...
    const safeNarrativeSections=input.narrativeSections.map(s=>({id:s.id,title:s.title,body:s.correctedBody??s.body,sourceId:s.sourceId}));
    const text=["Analyze unresolved conflicts conservatively, then create one guidebook draft proposal as JSON. Use only supplied confirmed facts and confirmed narrative sections. Never invent operational information or new claims beyond what is supplied. Confirmed narrative sections may be lightly edited for tone and consistency, but their substance must not change. Each section body must be concise plain text suitable for a canonical rich-text block.",...(input.instructions.allowImages===false?["Do not invent or describe media references. Images are attached after the canonical draft is saved."]:[]),`Template version: ${input.templateVersionId}`,`Allowed components: ${JSON.stringify(Object.fromEntries(input.allowedComponents))}`,`Presentation instructions: ${JSON.stringify(input.instructions)}`,`Confirmed facts: ${JSON.stringify(safeFacts)}`,`Confirmed narrative sections: ${JSON.stringify(safeNarrativeSections)}`].join("\n");
    ...
    try{const parsed=parseProviderProposal(output.text),componentVersions=[...input.allowedComponents].map(([key,versionId])=>({key,versionId})),proposal=guidebookProposalSchema.parse({...parsed,factIds:safeFacts.map(fact=>fact.id),narrativeSectionIds:safeNarrativeSections.map(section=>section.id),componentVersions});return{proposal,requestId:output.requestId,usage:output.usage,evidenceId}}catch(error){await input.evidence?.recordOutcome?.(evidenceId,{classification:"application_schema_failure",schemaValidationPaths:schemaPaths(error)});throw new CreationProviderError("invalid_output","The creation provider returned an invalid result.",true,null,output.usage,output.requestId)}
  }
...
function extractionInstruction(sources:readonly ProviderSource[]){return["Extract structured guidebook facts from the attached controlled sources. Return JSON with facts, missing, warnings, mediaCandidates, unsupported, and narrativeSections.","Do not invent facts or silently resolve conflicts. When the same field has different stated values, return every value as conflicting with the same non-empty conflictGroup. When the source explicitly says information is omitted, return that field in missing. Never mark provider-extracted facts confirmed; use needs_review unless the value is conflicting or missing.","Known access, Wi-Fi, safety, parking, occupancy, policy, and checkout facts are high risk. Classify maximum occupancy as rules.occupancy_limit with highRisk true. Never return permanent secret values: use null and needs_review.","Also identify whole descriptive or recommendation content that does not reduce to a single atomic fact — welcome letters, FAQ, safety tips, checklists, activity/dining/shopping recommendations, getting-around guidance, and contact/social information. Return each as a narrativeSections entry with a title and body preserving the source's substance (light cleanup only, no invented claims), reviewStatus always needs_review (never pre-confirmed, same rule as facts). Do not duplicate content already captured as an atomic fact in the same response (e.g. do not re-narrate a Wi-Fi password that is already a fact).",`Sources in attachment order: ${JSON.stringify(sources.map(s=>({id:s.id,type:s.sourceType,filename:s.originalFilename})))}`].join("\n")}
...
export function extractionOutputTokenAllowance(sources:readonly Pick<ProviderSource,"byteSize">[]){
  const totalBytes=sources.reduce((sum,source)=>sum+source.byteSize,0),
    expectedFacts=Math.min(500,Math.max(10,Math.ceil(totalBytes/500))),
    expectedNarrativeSections=Math.min(50,Math.max(0,Math.ceil(totalBytes/1500))),
    factBudget=2048+expectedFacts*128,
    narrativeBudget=expectedNarrativeSections*500;
  return Math.min(65536,Math.max(4096,factBudget+narrativeBudget));
}
