...
export default async function ExecuteActionPlanPage({ params,searchParams }: Readonly<{ params: Promise<{ planId: string }>;searchParams:Promise<{workspace?:string;from?:string}> }>) {
  ...
  const [plan, history] = await Promise.all([getExecutePlanAction({ planId,workspaceId:query.workspace }), getExecutePlanHistoryAction({ planId,workspaceId:query.workspace })]);
  if (!plan.ok) {
