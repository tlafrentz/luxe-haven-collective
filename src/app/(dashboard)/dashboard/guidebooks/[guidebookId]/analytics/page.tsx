...
export default async function GuidebookAnalyticsPage({ params }: Readonly<{ params: Promise<{ guidebookId: string }> }>) {
  ...
  const uniqueVisitors = summary.ok ? summary.value.uniqueVisitors : 0;
  const viewsByDay = summary.ok ? summary.value.viewsByDay : [];
  const maxDayCount = Math.max(1, ...viewsByDay.map((day) => day.count));
  return (
    <main className="mx-auto max-w-6xl space-y-6 py-8">
      ...
      <>
        <section aria-label="Engagement summary" className="grid gap-4 sm:grid-cols-3 lg:grid-cols-5">
          <Metric label="Total Views" value={counts.get("view")??0}/>
          <Metric label="QR Scans" value={counts.get("qr-scan")??0}/>
          <Metric label="Unique Guests" value={uniqueVisitors}/>
          <Metric label="Link interactions" value={(counts.get("link-click")??0)+(counts.get("map-open")??0)+(counts.get("phone-tap")??0)}/>
          <Metric label="Completed" value={counts.get("guidebook-completed")??0}/>
        </section>
        <section className="rounded-3xl border bg-white p-6">
          ...
          {viewsByDay.length ? (
              ...
              {viewsByDay.map((day) => (
                <div key={day.date} className="flex flex-1 flex-col items-center gap-2">
                  <div
                    ...
                    style={{ height: `${Math.max(4, (day.count / maxDayCount) * 100)}px` }}
            ...
            <p className="mt-3 text-sm text-stone-500">Confirmed zero views.</p>
          )}
        </section>
