...
export default async function ActionCenterPage({ searchParams }: { searchParams?: Promise<{ view?: string }> }) {
  const selectedView = parseActionCenterView((await searchParams)?.view);
  ...
  const [view, planResult] = await Promise.all([reader.loadQueue(query), selectedView === "plans" ? listExecutePlansAction() : Promise.resolve(null)]);
  return <ActionCenter view={view} selectedView={selectedView} plans={planResult?.ok ? planResult.value : []} />;
}
