...
import { FinancialOverviewErrorView, FinancialOverviewView, FinancialSnapshotForm } from "@/features/financial-intelligence/presentation";
...
export default async function FinancialOverviewPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return result.ok ? <><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-end gap-3 px-5 pt-5"><Link className="rounded-full border px-4 py-2 text-sm font-semibold" href="/dashboard/financial/expenses/new">Record expense</Link><Link className="rounded-full border px-4 py-2 text-sm font-semibold" href={`/dashboard/reports/new?type=financial-performance${single(params.workspace)?`&workspace=${single(params.workspace)}`:""}`}>Generate financial report</Link><FinancialSnapshotForm workspaceId={result.overview.identity.workspaceId} propertyIds={result.overview.scope.propertyIds} from={result.overview.period.from} to={result.overview.period.to}/></div><FinancialOverviewView overview={result.overview} /></> : <FinancialOverviewErrorView code={result.code} message={result.message} />;
}
