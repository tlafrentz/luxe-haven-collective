...
export default async function ReportPrintPage({params}:{params:Promise<{reportId:string}>}){const{reportId}=await params,view=await getGeneratedReportView(reportId);if(!view)notFound();return <main className="bg-white"><ReportViewer projection={view.report.projection_snapshot as ReportProjection} metadata={{reportNumber:view.report.report_number,generatedAt:view.report.generated_at,templateVersion:view.report.template_version}} /></main>;}
