...
export default async function NewReportPage({ searchParams }: { searchParams: Promise<{ sourceCapability?: string; sourceView?: string }> }) {
    ...
    <main className="mx-auto max-w-6xl space-y-6 px-5 py-8">
      ...
      {source ? (
        <section role="status" className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
          ...
          <p className="mt-2 text-sm text-stone-700">{view ? `Requested view: ${view}. ` : ""}Reports owns generation, status, artifacts, downloads, and history. Select an available definition below.</p>
        </section>
      ) : null}
