...
export async function storeInvestmentAnalysis(ownerId: string, result: InvestmentWorkspaceAnalysisResult, input: StoredAnalysis["input"], analyzedAt: Date): Promise<IssuedInvestmentAnalysisSaveToken> {
  ...
  console.info("investment_analysis_token_created",{requestId:result.lineage.workspaceRunId,actorId:ownerId,expiresAt:expiresAt.toISOString()});
...
export async function resolveInvestmentAnalysis(token: string, ownerId: string): Promise<StoredAnalysis | null> {
  ...
  if (!data) { console.info("investment_analysis_token_rejected",{actorId:ownerId,errorCode:"TOKEN_INVALID_OR_EXPIRED"}); return null; } const row = data as unknown as { owner_id: string; payload: { result: InvestmentWorkspaceAnalysisResult; input: StoredAnalysis["input"] }; analyzed_at: string; expires_at: string };
