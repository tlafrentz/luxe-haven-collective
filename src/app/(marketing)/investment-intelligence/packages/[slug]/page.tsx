...
export function generateStaticParams() {
  return investmentPackages.map((pkg) => ({ slug: pkg.slug }));
}
...
export default async function InvestmentPackageDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
      ...
      <section className="border-b border-[#dce2dd] bg-[#f6f3eb] py-16">
        <div className="container-shell">
          <nav className="text-xs text-stone-500">
            <Link href="/investment-intelligence/packages">Compare Packages</Link>
            <span className="mx-2">›</span>
            <span>{pkg.name}</span>
          </nav>
          <p className="mt-6 text-xs font-bold uppercase tracking-[.16em] text-[#a56b19]">
            Per analysis · {pkg.priceLabel}
          </p>
          ...
          <div className="space-y-10">
            ...
            <div className="grid gap-6 sm:grid-cols-2">
              <div className="rounded-xl border border-[#dce2dd] bg-white p-5">
                <h3 className="font-serif text-xl">Scope &amp; turnaround</h3>
                <dl className="mt-4 space-y-2 text-sm">
                  <div className="flex justify-between gap-4">
                    <dt className="text-stone-500">Market analysis</dt>
                    <dd className="text-right font-medium">{pkg.comparison.marketAnalysis}</dd>
                  </div>
                  <div className="flex justify-between gap-4">
                    <dt className="text-stone-500">Financial modeling</dt>
                    <dd className="text-right font-medium">{pkg.comparison.financialModeling}</dd>
                  </div>
                  <div className="flex justify-between gap-4">
                    <dt className="text-stone-500">Comparable properties</dt>
                    <dd className="text-right font-medium">{pkg.comparison.comparableProperties}</dd>
                  </div>
                  <div className="flex justify-between gap-4">
                    <dt className="text-stone-500">Scenarios modeled</dt>
                    <dd className="text-right font-medium">{pkg.comparison.scenarios}</dd>
                  </div>
                  <div className="flex justify-between gap-4">
                    <dt className="text-stone-500">Turnaround</dt>
                    <dd className="text-right font-medium">{pkg.comparison.turnaround}</dd>
                  </div>
                </dl>
              </div>
              <div className="rounded-xl border border-[#dce2dd] bg-white p-5">
                ...
                <p className="mt-4 text-xs text-stone-500">{pkg.eligibilityLimitations}</p>
              </div>
            </div>

            <div className="rounded-xl border border-[#dce2dd] bg-white p-5">
              <h3 className="font-serif text-xl">Payment, cancellation &amp; refunds</h3>
              <p className="mt-3 text-sm leading-6 text-stone-600">
                Payment is collected in full at checkout. Your analysis
                credit is fulfilled once your report is generated — this fee
                covers the analysis service, not any property purchase or
                transaction.
              </p>
              <p className="mt-3 text-sm leading-6 text-stone-600">
            ...
            <div>
              ...
              <div className="mt-5 grid gap-5 sm:grid-cols-2">
                {relatedPackages.map((related) => (
                  <Link
                    key={related.slug}
                    href={`/investment-intelligence/packages/${related.slug}`}
          ...
          <aside className="h-fit lg:sticky lg:top-28">
            <div className="overflow-hidden rounded-[1.5rem]">
              <div className="relative aspect-[4/5]">
                <Image
                  src={heroImage}
                  alt={`${pkg.name} investment analysis package example`}
                  fill
