...
export default async function FurnishingActivationAdminPage({searchParams}:{searchParams:Promise<{workspaceId?:string}>}){
 await requireRole(["admin"]);const db=await createClient(),{workspaceId=""}=await searchParams,names=["catalog_viewing","design_workspace","budgeting","procurement_readiness"];
 const[{data:release},{data:audit},{data:workspaceResolution},...capabilityResolutions]=await Promise.all([
  ...
  workspaceId?db.rpc("resolve_furnishing_activation_control",{p_target:"workspace",p_target_id:workspaceId,p_tenant_id:workspaceId}):Promise.resolve({data:null}),
  ...names.map(name=>workspaceId?db.rpc("resolve_furnishing_activation_control",{p_target:"capability",p_target_id:name,p_tenant_id:workspaceId}):Promise.resolve({data:null})),
 ]);
 const resolved=workspaceResolution as null|{status?:string;version?:number;state?:string;killSwitch?:boolean;cohort?:string|null;expiresAt?:string|null;revokedAt?:string|null},workspaces=resolved?.status==="found"?[{id:workspaceId,workspace_id:workspaceId,enabled:resolved.state==="internal",kill_switch:resolved.killSwitch!==false,cohort:resolved.cohort,expires_at:resolved.expiresAt,revoked_at:resolved.revokedAt,optimistic_version:Number(resolved.version)}]:[];
 const required=names.map((name,index)=>{const value=capabilityResolutions[index]?.data as null|{version?:number;state?:string};return{capability:name,enabled:value?.state==="internal",optimistic_version:Number(value?.version??0)}});
