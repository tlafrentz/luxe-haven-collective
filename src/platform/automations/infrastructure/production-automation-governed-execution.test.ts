...
type CompositionInput = Parameters<typeof createProductionGovernedExecution>[0];
const dependencies = { repository: {}, definitions: {}, policy: {}, approvalAuthority: {}, ports: [{ capability: "execute", contractVersions: ["v1"] }], serviceActor: { actorId: "service", tenantId: "tenant", policyId: "policy", active: true, grants: [{ capability: "execute", commandType: "createDraftPlan", propertyIds: [] }] }, retryPolicy: { version: "retry-v1", maximumAttempts: 3, maximumElapsedMs: 60000, initialDelayMs: 1000, maximumDelayMs: 10000, jitterRatio: 0, retryableClassifications: ["retryable_failure"] } } as unknown as Omit<CompositionInput, "flags">;
