export { AutomationBuilder, type AutomationExecutionBuilderInput, type AutomationRuleBuilderInput } from "./automation-builder";
export { AutomationExecutor, type AutomationExecutorInput, type AutomationInvoker } from "./automation-executor";
export { type AutomationPolicy, type AutomationPolicyContext, type AutomationPolicyResult } from "./automation-policy";
export { AutomationPolicyRegistry } from "./automation-policy-registry";
export { AutomationSession, type AutomationSessionInput } from "./automation-session";
export * from "./automation-foundation";
export * from "./automation-trigger-processing";
export * from "./automation-trigger-projections";
