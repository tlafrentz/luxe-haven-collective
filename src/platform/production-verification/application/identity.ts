...
export class RegisterControlledVerificationIdentity {
  ...
  async execute(input: { actorId: string; identity: VerificationIdentityRegistration; fingerprint: string; correlationId: string }) {
    ...
    if (existing) { if (existing.fingerprint !== input.fingerprint) throw new ProductionVerificationError("CONTROLLED_IDENTITY_DRIFT"); return Object.freeze({ registered: false }); }
    await this.repository.register({ ...input.identity, fingerprint: input.fingerprint }, input.actorId, input.correlationId); return Object.freeze({ registered: true });
  }
}
