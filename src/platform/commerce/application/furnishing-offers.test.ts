...
const input = (offerId: string, extra: Record<string, unknown> = {}) => ({ offerId, actor, activation, resolveActivation: (context: { globalKillSwitch:boolean; globalState:string; workspaceKillSwitch:boolean; workspaceEnabled:boolean; cohortEligible:boolean; capabilityEnabled:boolean }) => ({ allowed: !context.globalKillSwitch && context.globalState !== "disabled" && !context.workspaceKillSwitch && context.workspaceEnabled && context.cohortEligible && context.capabilityEnabled, reason: "enabled" }), providerReferences: { "FS-CONSULT": provider, "FS-DESIGN": provider }, ...extra } as Parameters<typeof resolveApprovedFurnishingOffer>[0]);
