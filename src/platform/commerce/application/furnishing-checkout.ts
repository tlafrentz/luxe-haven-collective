...
export class CreateFurnishingCheckout {
  constructor(private readonly dependencies: Readonly<{ repository: FurnishingCheckoutRepository; authorization: FurnishingCheckoutAuthorization; provider: FurnishingCheckoutProvider; hashIdempotency: (value: string) => Promise<string>; resolveActivation: (context: FurnishingActivationContext) => FurnishingActivationContext; activation: FurnishingActivationContext; providerReferences: Readonly<Record<string, FurnishingProviderReference | undefined>>; now?: () => Date; ttlSeconds?: number }>) {}
  async execute(input: Readonly<{ actor: FurnishingOfferActor; workspaceId: string; offerId: string; offerVersion?: number; idempotencyKey: string; returnContext?: "checkout" | "purchase" }>): Promise<FurnishingCheckoutResult> {
    ...
    const decision = resolveApprovedFurnishingOffer({ offerId: input.offerId, requestedVersion: input.offerVersion, workspaceId: input.workspaceId, actor: input.actor, activation: this.dependencies.resolveActivation(this.dependencies.activation), providerReferences: this.dependencies.providerReferences });
