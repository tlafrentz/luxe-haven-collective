...
export async function approveFs008dPackageVersion(db: SupabaseClient, input: Readonly<{ packageVersionId: string; expectedVersion: number; reason: string; correlationId: string; idempotencyKey: string }>) {
...
export async function createFs008dProjectCatalogSnapshot(db: SupabaseClient, input: Readonly<{ projectId: string; packageVersionId: string; snapshot: Record<string, unknown>; contentHash: string; correlationId: string; idempotencyKey: string }>) {
