export type FurnishingGlobalState =
  | "disabled"
  | "internal"
  | "limited"
  | "enabled"
...
export type FurnishingActivationDecision = Readonly<{
  allowed: boolean;
  reason: FurnishingDecisionReason;
  policyVersion: string;
  evaluatedAt: string;
}>;

/**
 * Canonical server-side policy. Lower controls are never evaluated as
 * overrides. This is the platform-importable copy: `src/platform/` code is
 * architecturally forbidden from importing `src/features/*`, so this lives
 * here and `src/features/furnishing-studio/activation.ts` re-exports it
 * (the reverse direction -- features importing platform -- is allowed).
 */
export function resolveFurnishingActivation(
  ...
  let reason: FurnishingDecisionReason = "enabled";
  if (!context.configurationValid) reason = "configuration_invalid";
    ...
    !context.actorRole ||
    !context.tenantRelationship ||
    context.tenantRelationship === "wrong_tenant" ||
    context.tenantRelationship === "none"
  )
    reason = "unauthorized";
  else if (context.entitlementRequired && !context.entitlementActive)
    reason = "entitlement_required";
  return Object.freeze({
    allowed: reason === "enabled",
    reason,
