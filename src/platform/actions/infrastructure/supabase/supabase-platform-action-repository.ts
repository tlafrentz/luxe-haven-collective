...
import { PlatformActionPersistenceFailure, StalePlatformActionVersion } from "../../application/platform-action-provider-errors";
...
export class SupabasePlatformActionRepository implements PlatformActionRepository {
  ...
  public async find(input: PlatformActionQuery): Promise<PlatformActionCollection> {
    ...
    if (!Array.isArray(data)) throw new PlatformActionPersistenceFailure("Platform Action query returned an invalid payload.");
  ...
  public async replace(input: Readonly<{ action: PlatformAction; expectedVersion: ActionVersion }>): Promise<void> {
    ...
    catch (error) { if (error instanceof PlatformActionPersistenceFailure && error.metadata?.databaseCode === "40001") throw new StalePlatformActionVersion(input.action.id.value, input.expectedVersion.value, { cause: error }); throw error; }
  }
  private async rpc(name: string, parameters: Readonly<Record<string, unknown>>): Promise<unknown> {
    ...
    if (error) throw new PlatformActionPersistenceFailure(error.message, { cause: error, metadata: { databaseCode: error.code, function: name } });
    return data;
  }
}
