...
Write findings to `docs/QA/<YYYY-MM-DD>-<scope-slug>.md` (e.g. `docs/QA/2026-08-03-marketing.md`) — this repo's existing docs directory already uses `docs/QA/` (capital, alongside other walkthrough/review docs), not lowercase `docs/qa/`; match it exactly since a case-different path is a real miss on case-sensitive filesystems (Linux CI) even though it silently works on macOS. Use this structure:
...
- **`read_page` with `filter: "interactive"` can silently under-report on content-heavy pages** — it returned only the 9 header nav links on the homepage and missed every CTA link in the body (confirmed present via `filter: "all"`), and returned nothing at all on `/hpm`. If a page has visible links/buttons that `filter: "interactive"` doesn't list, re-run with `filter: "all"` and a larger `max_chars` before concluding a control is missing — don't report a false "no link found" from the interactive filter alone.
- **A route flagged `auth-required` by the discovery script isn't always correctly gated.** `/reports/shared/[token]` is a public, token-based share page (no login intended) but got caught by the `/reports` prefix in `protectedRoutes` and redirected to `/login` anyway — a real bug, not a script error. Don't assume every `auth-required` tag reflects intentional design; if a route's own purpose (public share link, webhook, etc.) seems at odds with requiring a login, it's worth checking live even without credentials, since the redirect-or-not behavior itself is visible either way.
- **`docs/QA/` already existed in this repo before this skill did**, alongside other walkthrough/review docs — don't recreate it as lowercase `docs/qa/` (macOS's case-insensitive filesystem will silently merge the two locally, but it's a real miss on case-sensitive systems like Linux CI).
