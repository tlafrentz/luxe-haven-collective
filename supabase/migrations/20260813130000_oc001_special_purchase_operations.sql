...
revoke all on function public.claim_oc001_hpm_property_addon(uuid,uuid,uuid,integer,text,text),public.complete_oc001_hpm_property_addon(uuid,text,integer,text,text),public.claim_oc001_guidebook_hosting_renewal(uuid,uuid,uuid,uuid,text,text,text),public.complete_oc001_guidebook_hosting_renewal_schedule(uuid,text,timestamptz),public.apply_oc001_guidebook_hosting_renewal(text,timestamptz,timestamptz,boolean),public.create_oc001_furnishing_approved_purchase_intent(uuid,uuid,integer,text,text,text),public.consume_oc001_furnishing_configuration(uuid,uuid),public.cancel_oc001_furnishing_configuration(uuid,uuid,integer,text),public.expire_oc001_guidebook_hosting(uuid,text) from public,anon,authenticated;
grant execute on function public.claim_oc001_hpm_property_addon(uuid,uuid,uuid,integer,text,text),public.complete_oc001_hpm_property_addon(uuid,text,integer,text,text),public.claim_oc001_guidebook_hosting_renewal(uuid,uuid,uuid,uuid,text,text,text),public.complete_oc001_guidebook_hosting_renewal_schedule(uuid,text,timestamptz),public.apply_oc001_guidebook_hosting_renewal(text,timestamptz,timestamptz,boolean),public.create_oc001_furnishing_approved_purchase_intent(uuid,uuid,integer,text,text,text),public.consume_oc001_furnishing_configuration(uuid,uuid),public.cancel_oc001_furnishing_configuration(uuid,uuid,integer,text),public.expire_oc001_guidebook_hosting(uuid,text) to service_role;
