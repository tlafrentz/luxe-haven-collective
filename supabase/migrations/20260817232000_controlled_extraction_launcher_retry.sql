alter table public.guidebook_controlled_extraction_launches
  ...
  drop constraint if exists guidebook_controlled_extraction_launches_verification_run_id_key;

update public.guidebook_controlled_extraction_launches
set status='closed_failed',closed_at=now()
where correlation_id='41d40564-6825-4be1-9995-a7ba03880f7a'
  and status='cleanup_required'
  and not exists(
    select 1 from public.production_verification_attempts a
    where a.executor_code='VERIFY_CONTROLLED_GUIDEBOOK_EXTRACTION_V2'
  );
...
begin
 ...
 where l.operation_code='controlled_guidebook_extraction_v2'
