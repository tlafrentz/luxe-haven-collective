-- Keep Guidebook property creation compatible with the canonical property
-- configuration trigger introduced by 20260801030000.
begin;
...
begin
  ...
  where p.owner_id=p_workspace_id
    and lower(regexp_replace(trim(coalesce(p.address_line_1,p.address,'')), '[^a-zA-Z0-9]+', '', 'g'))=normalized_address
  ...
  insert into public.properties
    (owner_id,name,slug,description,short_description,address,address_line_1,city,state,
  ...
  values
    (p_workspace_id,trim(p_name),lower(regexp_replace(trim(p_name),'[^a-zA-Z0-9]+','-','g'))||'-'||substr(gen_random_uuid()::text,1,6),
     '',nullif(trim(p_short_description),''),trim(p_address),trim(p_address),trim(p_city),trim(p_state),
