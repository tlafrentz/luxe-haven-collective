...
begin
  ...
  select id
  into message_id
  from public.guest_communication_messages
  where provider=p_provider
    and provider_native_message_id=p_provider_message_id;

  if message_id is null then
    message_id:='guest-message-provider-'||pg_catalog.encode(
      extensions.digest(p_provider||':'||p_provider_message_id,'sha256'),
      'hex'
    );
  ...
  if message_id is null then
    raise exception 'provider_message_canonical_resolution_failed';
  end if;

  for attachment,attachment_index in
    select value,ordinality-1
    from jsonb_array_elements(coalesce(p_attachments,'[]'::jsonb)) with ordinality
  loop
    provider_attachment_id:=coalesce(
      nullif(attachment->>'providerAttachmentId',''),
      p_provider_message_id||':attachment:'||attachment_index
    );
    attachment_url:=coalesce(
      nullif(attachment->>'providerUrl',''),
      nullif(attachment->>'url','')
    );
    attachment_id:='guest-attachment-provider-'||pg_catalog.encode(
      extensions.digest(p_provider||':'||provider_attachment_id,'sha256'),
      'hex'
    );
    ...
    insert into public.guest_communication_attachments(
      ...
      coalesce(nullif(attachment->>'filename',''),nullif(attachment->>'name',''),'Provider attachment'),
      attachment_url,nullif(attachment->>'mimeType',''),
      nullif(attachment->>'sizeBytes','')::bigint,p_provider,
      provider_attachment_id,attachment_url,coalesce(attachment->'metadata','{}')
    ) on conflict(provider,provider_attachment_id)
      where provider is not null and provider_attachment_id is not null
      do nothing;
  end loop;

  if message_inserted then
    insert into public.guest_message_delivery_events(
      ...
      'delivery-provider-'||pg_catalog.encode(
        extensions.digest(p_provider||':'||p_provider_message_id,'sha256'),
        'hex'
      ),
    ...
    if not p_backfill then
      insert into public.guest_conversation_activity(
        ...
        'activity-provider-'||pg_catalog.encode(
          extensions.digest(p_provider||':'||p_provider_message_id,'sha256'),
          'hex'
        ),
...
revoke all on function public.ingest_guest_provider_message(
  uuid,uuid,uuid,text,text,text,text,text,text,text,text,text,text,text,text,text,
  timestamptz,timestamptz,jsonb,jsonb,jsonb,boolean
) from public;
