...
Status: AU-001F.1 preparation, AU-001F.2 hosted rehearsal, and AU-001F.3 disabled production schema deployment completed; AU infrastructure is production-present and inert; activation remains blocked. This document is not an AU release approval.
...
The inert application baseline was superseded by the disabled production schema deployment at commit `3ab17557`, Vercel deployment `dpl_GC3W4Dhvxc2ax7THdeUu9u7mz4P2`. The four checksum-approved AU migrations are now present in production with both kill switches enabled and every enablement path disabled. See `docs/releases/au-001f3-production-schema-deployment.md`.

## Release candidate inventory

| Slice                           | Commit                             | Local evidence                                                                      | Promotion state                                |
| ------------------------------- | ---------------------------------- | ----------------------------------------------------------------------------------- | ---------------------------------------------- |
| AU-001A Foundation              | `2c4e0b93`                         | domain, lifecycle, authorization, repository, migration/RLS tests                   | schema deployed; capability disabled           |
| AU-001B Triggers and Scheduling | `bd2e1c99`                         | occurrence, replay, DST, lease, scheduler tests                                     | schema deployed; processing disabled           |
...
| AU-001F Rollout                 | `3320632a`, `c35919f5`, `3ab17557` | release policy, rehearsal, disabled deployment, RLS, inactivity, and smoke evidence | F.3 complete; activation blocked                |
...
| Migration rehearsal        | Four AU migrations in production-equivalent database                       | Passed; production schema deployment also complete | AU-001F.2 rehearsal and AU-001F.3 production evidence                                      | Retain prerequisite and checksum checks for recovery                                        |
| PostgreSQL RLS             | Direct role/property/tenant/RPC/export matrix                              | Passed for deployed AU schema                       | rollback-only production matrix proved owner/admin, tenant/property, anonymous, and service boundaries | Security completes command/approval/export matrix when adapters are enabled          |
...
| Flags and kill switches    | Independent, default off, server evaluated                                 | Passed in production                               | both kill switches explicitly true; no AU enablement/cohort variables; inactivity passed   | Do not change before AU-001F.4 approval                                                     |
...
| Smoke tests                | Safe internal fixtures across enabled classes                              | Passed for disabled infrastructure boundary        | public/auth boundaries, disabled workspace/export, schema/RLS, and inactivity verified     | Authenticated command-class smoke remains blocked until AU-001F.4                           |
...
- Release commit: `3ab17557396069202f405cfc93f4adcd6b662f1e`; disabled deployment `dpl_GC3W4Dhvxc2ax7THdeUu9u7mz4P2`.
- Rollback code target: prior inert deployment `dpl_7B9vvS6Jc5uQEvji7Q3tcaZim8uP`; schema recovery remains forward-only.
...
- Evidence index: this file, `docs/releases/au-001f2-hosted-rehearsal.md`, `docs/releases/au-001f3-production-schema-deployment.md`, the migration inventory, AU architecture records, `docs/runbooks/automation-operations.md`, and HPM readiness.
- The disabled deployment identifier is recorded; no AU activation or final-release approver signature exists.
...
AU-001F.1 preparation, AU-001F.2 hosted rehearsal, and AU-001F.3 disabled production schema deployment are complete. AU-001F.4–F.6 remain blocked. Do not enable the workspace, flags, cohorts, schedules, workers, templates, or dispatch; create a production release tag; or claim AU-001 complete until every remaining gate has direct environment evidence and HPM-001F final approval.
