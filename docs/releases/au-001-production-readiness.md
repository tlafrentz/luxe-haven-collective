...
| Full regression            | Tests, lint, typecheck, build, migration lint                              | Passed locally for AU-001E RC predecessor         | AU-001E handoff; rerun after AU-001F preparation                                          | Engineering                                                                                |
| Migration rehearsal        | Four AU migrations in production-equivalent database                       | **Blocked**                                       | repository SQL and static migration tests only                                            | Database owner records durations, locks, row counts, query plans, rerun/failure recovery   |
| PostgreSQL RLS             | Direct role/property/tenant/RPC/export matrix                              | **Blocked**                                       | migration-level tests do not substitute for live PostgreSQL                               | Security and database owners execute non-production matrix                                 |
| Application authorization  | Definition, trigger, approval, dispatch, reconcile, exports                | Passed locally; production composition unverified | AU domain/application suites                                                              | Security verifies authenticated staging actors                                             |
| Service actors             | Least-privilege adapter grants and revocation                              | **Blocked**                                       | contracts exist; production identities/configuration absent                               | Integration owner inventories each actor and executes denial tests                         |
| Owning adapters            | HPM, Execute, notifications, and every enabled command compatible          | **Blocked**                                       | AU-001E registry reports unconfigured adapters safely                                     | Owning-capability owners configure and verify explicit versions                            |
...
| Flags and kill switches    | Independent, default off, server evaluated                                 | Passed locally                                    | release policy tests; all production defaults false                                       | Release owner configures values only after gates                                           |
...
| Observability              | Dashboards, alerts, delivery tests, on-call ownership                      | **Blocked**                                       | AU-001E projection and runbooks exist; provider dashboards not configured                 | Operations configures dashboards and tests alert delivery                                  |
| Rollback                   | Flags, drain/quarantine, artifact rollback, prior-app/schema compatibility | **Blocked**                                       | documented plan below; no environment rehearsal                                           | Release + database owners perform timed rehearsal                                          |
...
- Evidence index: this file, the migration inventory, AU architecture records, `docs/runbooks/automation-operations.md`, and HPM readiness.
