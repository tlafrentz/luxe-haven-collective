...
The controlled operator may enter preflight with ordinary `selected` or `none` property access and no assignment to a future fixture. After the claimed property is ledgered, the authenticated controlled owner uses the existing workspace `change-access` command to add only that property to the operator's existing selected-property set. The claim ledger and a verification control record atomically retain the original access mode and property IDs. Booking creation requires the active claim-owned assignment. Cleanup deletes the booking, restores the operator's exact original access snapshot through the same workspace command, then deletes the property. Command receipts, workspace access activity, and sanitized PS-001D audit remain as mandatory evidence; no invitation or access notification is generated.
