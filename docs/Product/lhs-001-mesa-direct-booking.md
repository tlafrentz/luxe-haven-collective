...
- **Reconciliation**: a new cron (`/api/internal/hospitable-reconciliation`, daily — Vercel's Hobby plan caps cron frequency at once/day) wraps the pre-existing `runHospitableReservationSync` pull-and-upsert pipeline — no new reconciliation logic was needed, since that pipeline was already one-directional (Hospitable → local).
